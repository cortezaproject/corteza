...
func (f *ModuleField) decodeTranslationsOptionsBoolLabels(tt locale.ResourceTranslationIndex) {
	...
	var aux *locale.ResourceTranslation
	if aux = tt.FindByKey(strings.NewReplacer("{{bool}}", "true").Replace(LocaleKeyModuleFieldOptionsBoolLabel.Path)); aux != nil {
		f.setOptionKey(aux.Msg, "trueLabel")
	}
	if aux = tt.FindByKey(strings.NewReplacer("{{bool}}", "false").Replace(LocaleKeyModuleFieldOptionsBoolLabel.Path)); aux != nil {
...
func (m *ModuleField) encodeTranslationsOptionsBoolLabels() (out locale.ResourceTranslationSet) {
	...
	out = append(out, &locale.ResourceTranslation{
		Resource: m.ResourceTranslation(),
		Key:      strings.NewReplacer("{{bool}}", "true").Replace(LocaleKeyModuleFieldOptionsBoolLabel.Path),
		Msg:      m.Options.String("trueLabel"),
	})

	out = append(out, &locale.ResourceTranslation{
		Resource: m.ResourceTranslation(),
		Key:      strings.NewReplacer("{{bool}}", "false").Replace(LocaleKeyModuleFieldOptionsBoolLabel.Path),
