...
func drainIterator(ctx context.Context, iter dal.Iterator, mod *types.Module, f types.RecordFilter) (set types.RecordSet, outFilter types.RecordFilter, err error) {
	...
	nav, err := generatePageNavigation(ctx, iter, mod, f.Paging, set)
	if err != nil {
		return
	}
...
func generatePageNavigation(ctx context.Context, iter dal.Iterator, mod *types.Module, p filter.Paging, set types.RecordSet) (out types.RecordFilter, err error) {
	const (
		howMuchMore = 1000
	)
	var (
