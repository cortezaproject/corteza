...
param(
    ...
    [switch]$Fresh = $false,
    [switch]$Audit = $false,
    [ValidateSet("all", "one", "admin", "compose", "workflow", "reporter", "discovery", "privacy", "mgmt")]
    [string]$App = "all"
)
...
if ($CodeGen) {
    ...
    try {
        ...
        # Run legacy REST codegen (generates request/handler files from rest.yaml)
        Write-Host "`n---Running legacy REST codegen---" -ForegroundColor $Yellow
        if (-not (Test-Path $codegenPath)) {
            Write-Host "---Building corteza-codegen---" -ForegroundColor $Yellow
            Push-Location "cmd\codegen"
            & go build -o $codegenPath main.go
        ...
        & $codegenPath -v
        Write-Host "Legacy REST codegen completed" -ForegroundColor $Green
    ...
    # Run JS API client codegen
    Write-Host "`n---Running JS API client codegen---" -ForegroundColor $Cyan
    Push-Location (Join-Path $LibDir "js")
    try {
        ...
        yarn
        yarn codegen
        Write-Host "JS API client codegen completed successfully" -ForegroundColor $Green
    } catch {
        Write-Host "Failed to run JS API client codegen: $_" -ForegroundColor $Yellow
        Write-Host "  You may need to run 'cd lib/js && yarn && yarn codegen' manually"
    }
    Pop-Location
...
if ($InstallAll) {
    ...
    if ($App -eq "all") {
        ...
        $webSubdirs = Get-ChildItem -Path (Join-Path $ClientDir "web") -Directory
        foreach ($dir in $webSubdirs) {
            Write-Host "---Building $($dir.Name)---" -ForegroundColor $Yellow
            Push-Location $dir.FullName
            try {
                yarn 
                yarn cdeps
                yarn build
                Write-Host "$($dir.Name) built successfully" -ForegroundColor $Green
            } catch {
                Write-Host "Failed to build $($dir.Name): $_" -ForegroundColor $Red
                Pop-Location
                exit 1
            }
            Pop-Location
...
# Copy locale files into server locale src
Write-Host "`nCopying locale files..." -ForegroundColor $Cyan
$LocaleSrc = Join-Path $ProjectRoot "locale"
$LocaleDest = Join-Path $ServerDir "pkg\locale\src"
if (Test-Path $LocaleSrc) {
    Copy-Item -Path "$LocaleSrc\*" -Destination $LocaleDest -Recurse -Force
    Write-Host "Locale files copied to $LocaleDest" -ForegroundColor $Green
} else {
    Write-Host "Locale source not found: $LocaleSrc" -ForegroundColor $Yellow
}
