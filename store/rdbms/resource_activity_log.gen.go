...
// discovery.Filter and expects to receive a working squirrel.SelectBuilder
func (s Store) SearchResourceActivityLogs(ctx context.Context, f discovery.Filter) (discovery.ResourceActivitySet, discovery.Filter, error) {
	var (
		err error
		set []*discovery.ResourceActivity
	...
	return set, f, func() error {
		...
		if q, err = setOrderBy(q, sort, s.sortableResourceActivityLogColumns()); err != nil {
			return err
		}
...
func (s Store) fetchFullPageOfResourceActivityLogs(
	...
	reqItems uint,
	check func(*discovery.ResourceActivity) (bool, error),
	cursorCond func(*filter.PagingCursor) squirrel.Sqlizer,
) (set []*discovery.ResourceActivity, prev, next *filter.PagingCursor, err error) {
	var (
		aux []*discovery.ResourceActivity
	...
	set = make([]*discovery.ResourceActivity, 0, DefaultSliceCapacity)
...
func (s Store) QueryResourceActivityLogs(
	...
	q squirrel.Sqlizer,
	check func(*discovery.ResourceActivity) (bool, error),
) ([]*discovery.ResourceActivity, error) {
	var (
		tmp = make([]*discovery.ResourceActivity, 0, DefaultSliceCapacity)
		set = make([]*discovery.ResourceActivity, 0, DefaultSliceCapacity)
		res *discovery.ResourceActivity
...
func (s Store) LookupResourceActivityLogByID(ctx context.Context, id uint64) (*discovery.ResourceActivity, error) {
...
func (s Store) CreateResourceActivityLog(ctx context.Context, rr ...*discovery.ResourceActivity) (err error) {
	for _, res := range rr {
...
func (s Store) UpdateResourceActivityLog(ctx context.Context, rr ...*discovery.ResourceActivity) error {
...
func (s Store) partialResourceActivityLogUpdate(ctx context.Context, onlyColumns []string, rr ...*discovery.ResourceActivity) (err error) {
	for _, res := range rr {
		err = s.checkResourceActivityLogConstraints(ctx, res)
		if err != nil {
			return err
...
func (s Store) UpsertResourceActivityLog(ctx context.Context, rr ...*discovery.ResourceActivity) (err error) {
	for _, res := range rr {
		err = s.checkResourceActivityLogConstraints(ctx, res)
		if err != nil {
			return err
...
func (s Store) DeleteResourceActivityLog(ctx context.Context, rr ...*discovery.ResourceActivity) (err error) {
	for _, res := range rr {
...
// returning discovery.ResourceActivity (or error)
func (s Store) execLookupResourceActivityLog(ctx context.Context, cnd squirrel.Sqlizer) (res *discovery.ResourceActivity, err error) {
	var (
		row rowScanner
	)
...
func (s Store) internalResourceActivityLogRowScanner(row rowScanner) (res *discovery.ResourceActivity, err error) {
	res = &discovery.ResourceActivity{}

	if _, has := s.config.RowScanners["resourceActivityLog"]; has {
		scanner := s.config.RowScanners["resourceActivityLog"].(func(_ rowScanner, _ *discovery.ResourceActivity) error)
		err = scanner(row, res)
	} else {
		err = row.Scan(
			&res.ID,
...
// internalResourceActivityLogEncoder encodes fields from discovery.ResourceActivity to store.Payload (map)
//
// Encoding is done by using generic approach or by calling encodeResourceActivityLog
// func when rdbms.customEncoder=true
func (s Store) internalResourceActivityLogEncoder(res *discovery.ResourceActivity) store.Payload {
...
//   undeleted items)
func (s Store) collectResourceActivityLogCursorValues(res *discovery.ResourceActivity, cc ...*filter.SortExpr) *filter.PagingCursor {
	var (
...
func (s *Store) checkResourceActivityLogConstraints(ctx context.Context, res *discovery.ResourceActivity) error {
