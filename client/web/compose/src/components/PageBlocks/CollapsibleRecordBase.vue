<template>
  ...
  >
    ...
    >
      ...
      >
        <!-- Body Field (Full Width - Rich Text display without toolbar) -->
        <div
...
export default {
  ...
  methods: {
     ...
     * Validates: only one of each marker, correct order (L before C before R)
     */
    parseAlignmentExpression (expression, record) {
      ...
      if (allMatches.length !== uniqueMarkers.length) {
        // Duplicate markers found - invalid
        return { hasValidMarkers: false, left: '', center: '', right: '' }
      }
      ...
      // Only validate if markers exist
      const hasL = lIndex !== -1
      const hasC = cIndex !== -1
      const hasR = rIndex !== -1
