...
func TestSessionTraceStepSetWalk(t *testing.T) {
	var (
		value = make(SessionTraceStepSet, 3)
		req   = require.New(t)
	)

	// check walk with no errors
	{
		err := value.Walk(func(*SessionTraceStep) error {
			return nil
		})
		req.NoError(err)
	}
	...
	// check walk with error
	req.Error(value.Walk(func(*SessionTraceStep) error { return fmt.Errorf("walk error") }))
}

func TestSessionTraceStepSetFilter(t *testing.T) {
	var (
		value = make(SessionTraceStepSet, 3)
		req   = require.New(t)
	)
	...
	// filter nothing
	{
		set, err := value.Filter(func(*SessionTraceStep) (bool, error) {
			return true, nil
		})
		req.NoError(err)
		req.Equal(len(set), len(value))
	...
	// filter error
	{
		_, err := value.Filter(func(*SessionTraceStep) (bool, error) {
			return false, fmt.Errorf("filter error")
		})
		req.Error(err)
	}
...
func TestSessionTraceStepSetIDs(t *testing.T) {
	var (
		value = make(SessionTraceStepSet, 3)
		req   = require.New(t)
	)
	...
	// construct objects
	value[0] = new(SessionTraceStep)
	value[1] = new(SessionTraceStep)
	value[2] = new(SessionTraceStep)
	// set ids
	value[0].ID = 1
	value[1].ID = 2
	value[2].ID = 3
