...
, PRIMARY KEY (id)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			require.New(t).Equal(strings.TrimSpace(tt.out), NewGenerator(zap.NewNop()).CreateTable(tt.in))
		})
	}
}
...
func Test_generator_CreateIndex(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			require.New(t).Equal(tt.out, NewGenerator(zap.NewNop()).CreateIndex(tt.in.Indexes[0]))
		})
	}
}
