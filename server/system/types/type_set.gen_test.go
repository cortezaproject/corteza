...
func TestRevisionSetWalk(t *testing.T) {
	var (
		value = make(RevisionSet, 3)
		req   = require.New(t)
	)

	// check walk with no errors
	{
		err := value.Walk(func(*Revision) error {
			return nil
		})
		req.NoError(err)
	}
	...
	// check walk with error
	req.Error(value.Walk(func(*Revision) error { return fmt.Errorf("walk error") }))
}

func TestRevisionSetFilter(t *testing.T) {
	var (
		value = make(RevisionSet, 3)
		req   = require.New(t)
	)
	...
	// filter nothing
	{
		set, err := value.Filter(func(*Revision) (bool, error) {
			return true, nil
		})
		req.NoError(err)
		req.Equal(len(set), len(value))
	...
	// filter error
	{
		_, err := value.Filter(func(*Revision) (bool, error) {
			return false, fmt.Errorf("filter error")
		})
		req.Error(err)
	}
...
func TestRevisionSetIDs(t *testing.T) {
	var (
		value = make(RevisionSet, 3)
		req   = require.New(t)
	)
	...
	// construct objects
	value[0] = new(Revision)
	value[1] = new(Revision)
	value[2] = new(Revision)
	// set ids
	value[0].ID = 1
	value[1].ID = 2
	value[2].ID = 3
...
func TestUserGroupSetWalk(t *testing.T) {
	var (
		value = make(UserGroupSet, 3)
		req   = require.New(t)
	)
...
func TestUserGroupSetFilter(t *testing.T) {
	var (
		value = make(UserGroupSet, 3)
		req   = require.New(t)
	)
...
func TestUserGroupSetIDs(t *testing.T) {
	var (
		value = make(UserGroupSet, 3)
		req   = require.New(t)
	)
