...
func SplitResourceIdentifier(ref string) (out map[string]Ref) {
	...
	case "corteza::system:ai-conversation":
		scope := Scope{}
	...
	case "corteza::system:llm-provider":
		...
		out["Path.0"] = Ref{
			ResourceType: "corteza::system:llm-provider",
			Identifiers:  MakeIdentifiers(gRef(pp, 0)),
			Scope:        scope,
		}

	case "corteza::system:notification":
		scope := Scope{}
		...
		out["Path.0"] = Ref{
			ResourceType: "corteza::system:notification",
			Identifiers:  MakeIdentifiers(gRef(pp, 0)),
			Scope:        scope,
		}
	...
	case "corteza::system:user-group":
		scope := Scope{}
