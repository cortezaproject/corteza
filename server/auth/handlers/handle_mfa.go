...
func (h *AuthHandlers) mfaForm(req *request.AuthReq) (err error) {
	...
	if req.Request.URL.Query().Get("action") == "resendEmailOtp" {
		...
		req.PushAlert(t("mfa.alerts.email.resent"))
	...
	req.Data["totpUnconfigured"] = req.AuthUser.UnconfiguredTOTP()
	return nil
}
...
func (h *AuthHandlers) mfaProc(req *request.AuthReq) (err error) {
	...
	case "verifyEmailOtp":
		...
		req.PushAlert(t("mfa.alerts.email.confirmed"))
	...
	case "verifyTotp":
		...
		req.PushAlert(t("mfa.alerts.totp.confirmed"))
