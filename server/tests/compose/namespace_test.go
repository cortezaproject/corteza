...
func TestNamespaceLabels(t *testing.T) {
	...
	t.Run("create", func(t *testing.T) {
		...
		helpers.SetLabelsViaAPI(h.apiInit(), t,
			"/namespace/",
			types.Namespace{Labels: map[string]labelTypes.LabelValue{"foo": {Val: "bar"}, "bar": {Val: "42"}}},
			payload,
		)
	...
	t.Run("update", func(t *testing.T) {
		...
		helpers.SetLabelsViaAPI(h.apiInit(), t,
			...
			&types.Namespace{Labels: map[string]labelTypes.LabelValue{"foo": {Val: "baz"}, "baz": {Val: "123"}}},
			payload,
		)
		req.NotZero(payload.ID)
