...
func TestWorkflowLabels(t *testing.T) {
	...
	t.Run("create", func(t *testing.T) {
		...
		helpers.SetLabelsViaAPI(h.apiInit(), t,
			"/workflows/",
			types.Workflow{Labels: map[string]labelTypes.LabelValue{"foo": {Val: "bar"}, "bar": {Val: "42"}}, Meta: &types.WorkflowMeta{Name: rs()}},
			payload,
		)
	...
	t.Run("update", func(t *testing.T) {
		...
		helpers.SetLabelsViaAPI(h.apiInit(), t,
			...
			&types.Workflow{Labels: map[string]labelTypes.LabelValue{"foo": {Val: "baz"}, "baz": {Val: "123"}}, Meta: &types.WorkflowMeta{Name: rs()}},
			payload,
		)
		req.NotZero(payload.ID)
