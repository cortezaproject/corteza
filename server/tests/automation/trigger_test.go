...
func TestTriggerLabels(t *testing.T) {
	...
	t.Run("create", func(t *testing.T) {
		...
		helpers.SetLabelsViaAPI(h.apiInit(), t,
			"/triggers/",
			types.Trigger{Labels: map[string]labelTypes.LabelValue{"foo": {Val: "bar"}, "bar": {Val: "42"}}, WorkflowID: wf.ID},
			payload,
		)
	...
	t.Run("update", func(t *testing.T) {
		...
		helpers.SetLabelsViaAPI(h.apiInit(), t,
			...
			&types.Trigger{Labels: map[string]labelTypes.LabelValue{"foo": {Val: "baz"}, "baz": {Val: "123"}}, WorkflowID: wf.ID},
			payload,
		)
		req.NotZero(payload.ID)
