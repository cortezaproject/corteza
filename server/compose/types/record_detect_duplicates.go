		...
		_ = vv.Walk(func(v *RecordValue) error {
			if v.RecordID != rec.ID {
				...
				_ = rvv.Walk(func(rv *RecordValue) error {
					if matchValue(c.Modifier, rv.Value, v.Value) {
				...
				if ((!c.HasMultiValue() || c.IsAllEqual()) && valErr.Len() == rvv.Len()) || (c.IsOneOf() && valErr.Len() > 0) {
					if out == nil {
						out = &RecordValueErrorSet{}
					}
					out.Push(valErr.Set...)
...
func matchValue(modifier DeDupValueModifier, input string, target string) bool {
	switch modifier {
	case ignoreCase:
		return str.Match(input, target, str.SimpleMatch)
	case caseSensitive:
		return str.Match(input, target, str.SimpleMatch)
	case fuzzyMatch:
