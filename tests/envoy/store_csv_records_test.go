...
// TestStoreCsv_records takes data from s1, encodes it into csv files, decodes
// created csv files, encodes into s2 and compares the data from s2.
func TestStoreCsv_records(t *testing.T) {
	type (
		tc struct {
			name string
			...
			postCsvEncode   func(req *require.Assertions, err error)
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			ss := ce.Stream()
			if c.postCsvEncode != nil {
				c.postCsvEncode(req, err)
			} else {
				req.NoError(err)
			}
...
func TestStoreCsv_records_fieldTypes(t *testing.T) {
	type (
		tc struct {
			name string
			// Before the data gets processed
