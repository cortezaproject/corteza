  ...
  methods: {
    ...
    extractWorkflowErrorMeta (err) {
      ...
      for (const meta of candidates) {
        ...
        const body = meta['workflow.error.body']
      ...
      return (err = {}) => {
        ...
        try {
          ...
          if (wfErr) {
            ...
            // Prefer the rich body; fall back to the flat error message
            // (which the backend also sets to body when available).
            //
            // NOTE: body/title originate from workflow-author-controlled
            // meta fields. BootstrapVue's $bvToast.toast(content, opts)
            // renders both body and title as plain text (no v-html),
            ...
            const body = wfErr.body || err.message || ''
