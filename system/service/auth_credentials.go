...
//func (svc *auth) CheckPassword(cc types.CredentialsSet, password string) bool {
...
// checks if password is reused by looking all valid and invalid credentials
// and taking reuse time window into consideration
func (svc *auth) checkPasswordReuse(cc []*types.Credentials, password string) bool {
	return false
}
...
func isValidPassword(cc []*types.Credentials, password string) bool {
...
//
// should be used as a parameter for credentialsFilter fn
func isPasswordReused(cc []*types.Credentials, password string, reuseWindow time.Duration) bool {
...
//
// should be used as a parameter for credentialsFilter fn
func skipInvalid(c *types.Credentials) bool {
	return c.Valid()
}

//
func compareHashedCredentials(password string) func(c *types.Credentials) bool {
	var (
		p = []byte(password)
	)

	return func(c *types.Credentials) bool {
		return bcrypt.CompareHashAndPassword([]byte(c.Credentials), p) == nil
	}
}

func skipNewerCredentials(cutoff time.Duration) func(c *types.Credentials) bool {
	var (
		t = now().Add(cutoff * -1)
	)
...
func credentialsFilter(cc []*types.Credentials, limit int, mm ...func(*types.Credentials) bool) (out []*types.Credentials) {
